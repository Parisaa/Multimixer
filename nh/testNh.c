...
static uint64_t measureNhNeon(uint64_t dtMin, unsigned int inputLen){
    ...
    {
        dtMin = now();
	    Nhfield(input, key, output, (size_t)inputLen);
        dtMin=now()-dtMin;
    }
}
...
void testNhOne( void )
{
    ...
    for(len=0; len <= 11; len=len+1) {
        ...
        for(int i = 0; i <=1000000; i++){
            calibration = measureNhNeon(calibration,32*length[len]);// 32*len);
            if (calibration < MinT){
                MinT=calibration;
                }
        }
